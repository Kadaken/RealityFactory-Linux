...
CAnimGif::CAnimGif(const char *szFile, int fileformat)
{
	...
	if(geVFile_Read(MainFS, buffer, 13) == GE_TRUE)
	{
		...
		{
			nWidth = rf_read_le16(buffer+6);
			nWidth = ((nWidth-1)|0x3)+1;
			nHeight = rf_read_le16(buffer+8);
			...
			GifSize = Size-3*GlobalColorSize-13;
...
l1:
	...
	case 0x21:
		...
		case 0xF9:
			...
			DelayTime = rf_read_le16(pcGifTrack);
			pcGifTrack += 2;
...
bool CAnimGif::TakeIt(void)
{
	...
	iLeft = rf_read_le16(pcGifTrack);
	pcGifTrack += 2;

	iTop = rf_read_le16(pcGifTrack);
	pcGifTrack += 2;

	iWidth = rf_read_le16(pcGifTrack);
	pcGifTrack += 2;

	iWidth1 = ((iWidth-1)|0x3)+1;

	iHeight = rf_read_le16(pcGifTrack);
	pcGifTrack += 2;
