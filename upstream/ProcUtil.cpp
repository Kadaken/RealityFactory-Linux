					...
					*pDst++ = (pSrc[1] + pSrc[s])>>1;
					...
					for(y=h-2; y--;)
					{
						*pDst++ = (pSrc[-1] + pSrc[1] + pSrc[s] + pSrc[-s])>>2;
						...
						for(int blur_x=0; blur_x<x; ++blur_x)
							pDst[blur_x] = (pSrc[blur_x+1] + pSrc[blur_x-1] +
								pSrc[blur_x+s-1] + pSrc[blur_x+s] + pSrc[blur_x+s+1] +
								pSrc[blur_x-s-1] + pSrc[blur_x-s] + pSrc[blur_x-s+1]) >> 3;
