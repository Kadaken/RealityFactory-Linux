...
// changed RF064
	...
	m_nTimerID = timeSetEvent(1, 0,	&TimerFunction, (DWORD_PTR)this,
...
// 08.05.2004 - end change gekido
	...
	else
	{
		...
		const bool encrypted = memcmp(szInputString, "CF00", 4) == 0;
		VFS = PassWord(m_VirtualFile, encrypted);
		if (!VFS)
		{
			ReportError("[ERROR] Configured VFS archive could not be opened; check format and read access", false);
			exit(-1);
		}
		ReportError(encrypted ? "\n[INFO] VFS detected (encrypted)..." :
...
// end change RF063
	...
	// Stage 1 alone is render-only. Stages 2+ must construct every common
	// manager below in the normal upstream order.
	if (RF_HostBootStage() == 1) return 0;
...
// end multiplayer
	...
	CloseJoysticks(); // Release SDL joystick handles before the host's SDL_Quit.
...
// changed RF064
	ReportError("Initializing NPC Track Manager Subsystem...", false);
	theTrack = new CTrack();
	if(theTrack == NULL)
	{
		theGameEngine->ReportError("[ERROR] Failed to create NPC Track handler", false);
		return -40;
	}

	ReportError("Initializing NPC PathPoint Manager Subsystem...", false);
	theNPCPoint = new CNPCPathPoint();
	if(theNPCPoint == NULL)
	{
		theGameEngine->ReportError("[ERROR] Failed to create NPC PathPoint handler", false);
		return -40;
	}

	ReportError("Initializing NonPlayerCharacter Manager Subsystem...", false);
	theNPC = new CNPC();
	if(theNPC == NULL)
	{
		theGameEngine->ReportError("[ERROR] Failed to create NonPlayerCharacter handler", false);
		return -40;
	}
...
// end multiplayer
	...
	SAFE_DELETE(theNPCPoint);
	SAFE_DELETE(theTrack);
	SAFE_DELETE(theNPC);
...
// end change RF064
		...
		case RGF_K_WEAPON_0:
			...
			break;
		case RGF_K_WEAPON_NEXT:
			theWeapon->CycleWeapon(1);
			break;
		case RGF_K_WEAPON_PREVIOUS:
			theWeapon->CycleWeapon(-1);
			break;
		case RGF_K_FIRE:
...
int CCommonData::PollJoystickAxis(int jn, int a)
{
	if(!m_bUseDInput)
		return -1;
	const int joystick_count = (int)(sizeof(joysticks) / sizeof(joysticks[0]));
	if(jn < 0 || jn >= joystick_count || joysticks[jn] == NULL)
		return -1;
...
bool CCommonData::CheckJoystickButton(int jn, int bn)
{
	if(!m_bUseDInput)
		return false;
	const int joystick_count = (int)(sizeof(joysticks) / sizeof(joysticks[0]));
	if(jn < 0 || jn >= joystick_count || bn < 0 ||
		bn >= (int)sizeof(((DIJOYSTATE2 *)0)->rgbButtons) || joysticks[jn] == NULL)
		return false;
...
void CCommonData::CloseJoysticks()
{
	...
	unsigned int i;
	const unsigned int joystick_count = sizeof(joysticks) / sizeof(joysticks[0]);

    // Close every owned slot even if a device was unplugged after startup.
...
// end change
	...
	for (char *separator = szTemp; *separator; ++separator)
		if (*separator == '\\') *separator = '/';
	FILE *pTemp = fopen(szTemp, szHow);
	if (!pTemp) {
		char resolved[PATH_MAX];
		if (rf_case_path(szTemp, resolved, sizeof(resolved), nFileType) == 1)
			pTemp = fopen(resolved, szHow);
	}
...
// end change
	...
	*theFp = geVFile_OpenNewSystem(NULL, GE_VFILE_TYPE_DOS, szTemp, NULL, nHow);
	if (!*theFp) {
		char resolved[PATH_MAX];
		int matched = rf_case_path(szTemp, resolved, sizeof(resolved), nFileType);
		if (matched < 0) return false; // Do not mask an ambiguity with a VFS hit.
		if (matched == 1)
			*theFp = geVFile_OpenNewSystem(NULL, GE_VFILE_TYPE_DOS, resolved, NULL, nHow);
	}
...
// end change
		// Archive lookup is case-insensitive, but requires backslashes. Keep
		// szTemp untouched; POSIX case resolution above uses a separate buffer.
...
{
	if (RF_BootGameActive())
	{
		LastTimePassed_F = RF_BootGameStepMilliseconds();
		LastTimePassed_D = (DWORD)LastTimePassed_F;
		return LastTimePassed_F;
	}
