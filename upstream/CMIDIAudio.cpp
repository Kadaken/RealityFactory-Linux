...
CMIDIAudio::CMIDIAudio()
{
	...
	m_savefile[0] = 0;
	m_saveloop = false;
	CCD->ReportError("[unavailable] CMIDIAudio: native MIDI playback is not implemented", false);
}
...
{
	CCD->ReportError("[unavailable] CMIDIAudio::Play", false);
	if (szFile == NULL)
		return RGF_FAILURE;
	strncpy(m_savefile, szFile, sizeof(m_savefile) - 1);
	m_savefile[sizeof(m_savefile) - 1] = 0;
	m_saveloop = bLoop;
	return RGF_FAILURE;
}
...
{
	CCD->ReportError("[unavailable] CMIDIAudio::PlayList", false);
	(void)szList;
	(void)nCount;
	return RGF_FAILURE;
}
...
bool CMIDIAudio::IsPlaying() { return false; }
...
int CMIDIAudio::SaveTo(FILE *SaveFD)
{
	...
	fwrite(&empty_list, sizeof(int), 1, SaveFD);
	fwrite(&m_Position, sizeof(int), 1, SaveFD);
	fwrite(m_MIDIFile, 1, sizeof(m_MIDIFile), SaveFD);
	return RGF_SUCCESS;
}
...
int CMIDIAudio::RestoreFrom(FILE *RestoreFD)
{
	...
	int saved_position;
	char saved_file[256];
	fread(&saved_looping, sizeof(bool), 1, RestoreFD);
	fread(&saved_active, sizeof(bool), 1, RestoreFD);
	fread(&saved_count, sizeof(int), 1, RestoreFD);
	for (int i = 0; i < saved_count; ++i) {
		int length = 0;
		fread(&length, sizeof(int), 1, RestoreFD);
		if (length > 0)
			fseek(RestoreFD, length, SEEK_CUR);
	}
	fread(&saved_position, sizeof(int), 1, RestoreFD);
	fread(saved_file, 1, sizeof(saved_file), RestoreFD);
	(void)saved_looping;
	(void)saved_active;
	(void)saved_position;
	return RGF_SUCCESS;
