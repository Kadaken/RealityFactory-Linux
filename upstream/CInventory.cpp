...
// changed QD
			...
			if(Tname.empty() && AttrFile.GetValue(KeyName, "arrowrhighlight").empty() &&
				AttrFile.GetValue(KeyName, "arrowl").empty() &&
				AttrFile.GetValue(KeyName, "arrowlhighlight").empty())
				fprintf(stderr, "inventory: arrow bitmaps absent (pre-0.68 config)\n");
...
// change QD
			if(arrowr) geEngine_AddBitmap(CCD->Engine()->Engine(), arrowr);
			if(arrowl) geEngine_AddBitmap(CCD->Engine()->Engine(), arrowl);
			if(arrowrhighlight) geEngine_AddBitmap(CCD->Engine()->Engine(), arrowrhighlight);
			if(arrowlhighlight) geEngine_AddBitmap(CCD->Engine()->Engine(), arrowlhighlight);
// end change QD
		}
		else
		{
...
void CInventory::Blit(int menu)
{
	...
	if((menu == WEAPONINV && MaxWeapons > 9) || (menu == ITEMINV && MaxItems > 9))
	{
		if(Selected == 9)
		{
			if(arrowrhighlight) geEngine_DrawBitmap(CCD->Engine()->Engine(), arrowrhighlight, NULL, backgroundx+arrowrx, backgroundy+arrowry);
		}
		else
		{
			if(arrowr) geEngine_DrawBitmap(CCD->Engine()->Engine(), arrowr, NULL, backgroundx+arrowrx, backgroundy+arrowry);
		}

		if(Selected == -1)
		{
			if(arrowlhighlight) geEngine_DrawBitmap(CCD->Engine()->Engine(), arrowlhighlight, NULL, backgroundx+arrowlx, backgroundy+arrowly);
		}
		else
		{
			if(arrowl) geEngine_DrawBitmap(CCD->Engine()->Engine(), arrowl, NULL, backgroundx+arrowlx, backgroundy+arrowly);
		}
	}
}
