...
#include <assert.h>
#include <stdio.h>
#include "../adapters/rf_platform_compat.h"
#ifndef _WIN32
#include <SDL2/SDL.h>
#endif
...
static geBoolean DrvList_ModeExistsOnAnyDisplay(int Width, int Height)
{
	int DisplayCount;
	int DisplayIndex;
	...
	for(DisplayIndex = 0; DisplayIndex < DisplayCount; ++DisplayIndex)
	{
		...
		for(ModeIndex = 0; ModeIndex < ModeCount; ++ModeIndex)
		{
			...
			if(SDL_GetDisplayMode(DisplayIndex, ModeIndex, &DisplayMode) == 0 &&
			   DisplayMode.w == Width && DisplayMode.h == Height)
				return GE_TRUE;
		}
	}
...
static int DrvList_ModeDistance(const ModeList *Mode,
								int DesktopWidth, int DesktopHeight)
{
	int WidthDelta = Mode->Width - DesktopWidth;
	int HeightDelta = Mode->Height - DesktopHeight;
...
/*
 * The native runtime has no Win32 driver-selection dialog. Choose a mode
 * deterministically from Genesis3D's enumerated modes, preferring modes SDL
 * confirms exist on a physical display and then the mode nearest display 0's
 * desktop size. If SDL video is not initialized yet, preserve the engine's
 * sorted-list automatic choice.
 */
geBoolean DrvList_PickDriver(HANDLE hInstance, HWND hwndParent,
							 ModeList *List, int ListLength, int *ListSelection)
{
	SDL_DisplayMode DesktopMode;
	geBoolean HaveDesktopMode = GE_FALSE;
	int BestSelection = -1;
	int BestClass = 0x7fffffff;
	...
	if(SDL_WasInit(SDL_INIT_VIDEO) != 0 &&
	   SDL_GetCurrentDisplayMode(0, &DesktopMode) == 0)
		HaveDesktopMode = GE_TRUE;
	...
	for(i = 0; i < ListLength; ++i)
	{
		...
		if(List[i].Evaluation == MODELIST_EVALUATED_TRIED_FAILED)
			continue;

		CandidateClass =
			(List[i].Evaluation == MODELIST_EVALUATED_OK) ? 0 : 2;
		if(HaveDesktopMode &&
		   !DrvList_ModeExistsOnAnyDisplay(List[i].Width, List[i].Height))
			CandidateClass += 1;

		CandidateDistance = HaveDesktopMode
			? DrvList_ModeDistance(&List[i], DesktopMode.w, DesktopMode.h)
			: i;

		if(CandidateClass < BestClass ||
		   (CandidateClass == BestClass && CandidateDistance < BestDistance))
		{
			BestSelection = i;
	...
	*ListSelection = BestSelection;
	return GE_TRUE;
}

/* Resize the borrowed native window without changing visibility or output. */
void ResetMainWindow(HWND hWnd, int32 Width, int32 Height)
{
	if (!hWnd || Width <= 0 || Height <= 0) {
		fputs("RF window resize requires a valid SDL window and positive dimensions\n", stderr);
		return;
	}
	SDL_SetWindowSize((SDL_Window *)hWnd, Width, Height);
}
