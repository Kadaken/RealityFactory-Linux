...
CCDAudio::CCDAudio()
{
	...
	m_savetrack = 0;
	m_cdon = false;
	CCD->ReportError("[unavailable] CCDAudio: native CD playback is not implemented", false);
}
...
{
	(void)nTrack; (void)bLoop;
	CCD->ReportError("[unavailable] CCDAudio::Play", false);
...
bool CCDAudio::IsPlaying() { return false; }

int CCDAudio::PlaySequence(int nCount, int *nTrackList)
{
	if (nCount <= 0 || nCount > 99 || nTrackList == NULL)
		return RGF_FAILURE;
	CCD->ReportError("[unavailable] CCDAudio::PlaySequence", false);
	return RGF_UNIMPLEMENTED;
}
