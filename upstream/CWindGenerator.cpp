	...
	if(m_CurrentTime > m_ThinkTime)
	{
		...
		case 0: //beginning
			...
			if(geVec3d_Compare(&Wind, &m_DestWindSpeed, m_Tolerance))
			{
				...
				InitialWind = CCD->Player()->GetInitialWind();
				m_DestWindSpeed.X = EffectC_Frand(m_MinWindSpeed.X, m_MaxWindSpeed.X) + InitialWind.X;
				m_DestWindSpeed.Y = EffectC_Frand(m_MinWindSpeed.Y, m_MaxWindSpeed.Y) + InitialWind.Y;
				m_DestWindSpeed.Z = EffectC_Frand(m_MinWindSpeed.Z, m_MaxWindSpeed.Z) + InitialWind.Z;
