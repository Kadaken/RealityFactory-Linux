...
{
    RF_HostGameplayScope hostMenu(0);
    RF_HostMenuScope hostSoftwareCursor;
...
// 08.05.2004 - end change gekido
	...
	for(;;)
	{
		...
		if (RF_HostQuitRequested()) return 0;
		if(!CCD->GetHasFocus())
			continue;
...
// end change RF063
		...
		if(!RF_HostMenuPointer(&temppos, CCD->Engine()->Width(), CCD->Engine()->Height()))
		{
			temppos.x = -1;
			temppos.y = -1;
		}
...
// end change
		...
		if (!geEngine_EndFrame(CCD->Engine()->Engine())) return -1;
		RF_HostBootFrame(3);
...
{
    RF_HostGameplayScope hostGameplay(1);
// 08.05.2004 - begin change gekido
...
// end change
			...
			const bool bootCaptured = RF_BootGameCapture();
			const int endResult = CCD->Engine()->EndFrame(); // All done, do the necessary flip
			if (!RF_BootGameEndFrame(bootCaptured, endResult)) break;
...
// end change
	...
	if(fd)
	{
		...
		// MenuInitalize also runs inside the constructor, before CCommonData
		// has assigned theMenu. Avoid the global-menu callback in that path.
		SetMusicVol(((float)((Vol_Slide.Current*100)/Vol_Slide.Max_X))*0.01f);
