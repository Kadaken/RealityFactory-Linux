...
void CInput::ScanKeyboardInput()
{
	...
	// SDL reports the wheel as discrete events rather than held virtual keys.
	// Preserve one action per detent and let later frames drain a fast wheel.
	const int wheel = RF_InputTakeWheelStep();
	if(wheel != 0 && m_KeyStackCount < (int)(sizeof(m_KeyStack)/sizeof(m_KeyStack[0])))
		m_KeyStack[m_KeyStackCount++] = wheel < 0 ? RGF_K_WEAPON_NEXT : RGF_K_WEAPON_PREVIOUS;
