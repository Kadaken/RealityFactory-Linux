...
// changed RF063
	...
	// D21: the old-animation scan only has meaning when a current weapon
	// exists. The inherited code ran it with CurrentWeapon == -1 on the first
	// SetWeapon of a level, indexing WeaponD[-1]. Same sentinel guard as above.
	if(Motion != NULL && !(CurrentWeapon == -1 || CurrentWeapon == MAX_WEAPONS))
	{
...
/* Select the next owned/loaded weapon slot for one SDL wheel detent. */
int CWeapon::CycleWeapon(int direction)
{
	if(direction == 0)
	...
	int currentSlot = direction > 0 ? -1 : 0;
	if(CurrentWeapon >= 0 && CurrentWeapon < MAX_WEAPONS)
		currentSlot = WeaponD[CurrentWeapon].Slot;

	for(int offset = 1; offset <= MAX_WEAPONS; ++offset)
	{
		int slot = currentSlot + (direction > 0 ? offset : -offset);
		while(slot < 0) slot += MAX_WEAPONS;
		slot %= MAX_WEAPONS;
		const int candidate = Slot[slot];
		if(candidate >= 0 && candidate < MAX_WEAPONS && WeaponD[candidate].active
			&& inventory->Has(WeaponD[candidate].Name) == RGF_SUCCESS
			&& inventory->Value(WeaponD[candidate].Name) > 0)
		{
			SetWeapon(slot);
			return CurrentWeapon;
		}
