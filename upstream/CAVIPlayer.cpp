...
CAVIPlayer::CAVIPlayer() { Init(); }
CAVIPlayer::~CAVIPlayer() { Release(); }
int CAVIPlayer::Play(const char *, int, int, bool) { return RGF_FAILURE; }
int CAVIPlayer::Open(const char *) { return RGF_FAILURE; }
int CAVIPlayer::DisplayFrameAt(int, int, DWORD) { return RGF_FAILURE; }
int CAVIPlayer::DisplayFrame(int, int, int) { return RGF_FAILURE; }
int CAVIPlayer::Close() { Release(); return RGF_SUCCESS; }
int CAVIPlayer::GetAudioStreamCount() { return 0; }
int CAVIPlayer::GetVideoStreamCount() { return 0; }
int CAVIPlayer::DisplayFrameTexture(int, const char *) { return RGF_FAILURE; }
int CAVIPlayer::DisplayNextFrameTexture(const char *, bool) { return RGF_FAILURE; }

void CAVIPlayer::Init()
{
	m_bVideoPrimed = false;
	m_pAviFile = NULL;
	memset(m_pAudioStreams, 0, sizeof(m_pAudioStreams));
	memset(m_pVideoStreams, 0, sizeof(m_pVideoStreams));
	m_nNumAudioStreams = 0;
	m_nNumVideoStreams = 0;
	memset(m_pAudioFormats, 0, sizeof(m_pAudioFormats));
	memset(m_pAudioData, 0, sizeof(m_pAudioData));
	memset(m_pVideoFormats, 0, sizeof(m_pVideoFormats));
	memset(m_pVideoPGF, 0, sizeof(m_pVideoPGF));
	memset(m_lVideoEndTime, 0, sizeof(m_lVideoEndTime));
	m_pDS = NULL;
	m_pStream = NULL;
	m_nOffset = 0;
	m_AudioEOF = false;
	m_nLastFramePlayed = -1;
	m_nBufSize = 0;
	m_LastFrameBitmap = NULL;
}

void CAVIPlayer::Release() { Init(); }
void CAVIPlayer::FindStreams() {}
bool CAVIPlayer::DetermineAudioFormats() { return false; }
bool CAVIPlayer::DetermineVideoFormats() { return false; }
int CAVIPlayer::ExtractAudioStream(int, int, LPBYTE) { return RGF_FAILURE; }
bool CAVIPlayer::StartVideoRetrieve(int) { return false; }
bool CAVIPlayer::EndVideoRetrieve(int) { return false; }
void CAVIPlayer::GetVideoFrameAtTime(int, LONG, LPBITMAPINFOHEADER *bitmap)
{
	if (bitmap != NULL) *bitmap = NULL;
}
void CAVIPlayer::GetVideoFrame(int, LONG, LPBITMAPINFOHEADER *bitmap)
{
	if (bitmap != NULL) *bitmap = NULL;
}
LPWAVEFORMATEX CAVIPlayer::GetAudioFormat(int) { return NULL; }
LPBITMAPINFO CAVIPlayer::GetVideoFormat(int) { return NULL; }
int CAVIPlayer::CreateStreamingAudioBuffer(int) { return RGF_FAILURE; }
int CAVIPlayer::DestroyStreamingAudioBuffer() { return RGF_SUCCESS; }
void CAVIPlayer::PumpBuffer(int, bool) {}
int CAVIPlayer::GetMaxWriteSize() { return 0; }
