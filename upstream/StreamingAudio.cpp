...
StreamingAudio::StreamingAudio(LPDIRECTSOUND lpDS)
{
	m_pDS = lpDS;
	m_hWaveFile = NULL;
	m_nDataPosition = m_nCurrentPosition = m_nDataSize = m_nDataLeft = 0;
	m_fActive = false;
	m_pStream = NULL;
	m_nTimerID = 0;
	m_pWaveFormat = NULL;
	memset(&m_rRiffData, 0, sizeof(m_rRiffData));
	m_fEOF = false;
	m_nOffset = 0;
	m_bLoops = false;
	mp3 = false;
...
StreamingAudio::~StreamingAudio() { Delete(); }
int StreamingAudio::Create(char *) { return RGF_FAILURE; }
int StreamingAudio::Play(bool) { return RGF_FAILURE; }
int StreamingAudio::Stop() { m_fActive = false; return RGF_SUCCESS; }
int StreamingAudio::Pause() { return RGF_SUCCESS; }
int StreamingAudio::Delete() { m_fActive = false; return RGF_SUCCESS; }
int StreamingAudio::Rewind() { return RGF_FAILURE; }
bool StreamingAudio::IsPlaying() { return false; }
int StreamingAudio::SetVolume(LONG) { return RGF_SUCCESS; }
DWORD StreamingAudio::GetMaxWriteSize() { return 0; }
int StreamingAudio::PumpWave(int) { return RGF_FAILURE; }
void CALLBACK StreamingAudio::TimerFunction(UINT, UINT, DWORD_PTR, DWORD_PTR, DWORD_PTR) {}
int StreamingAudio::WaveOpen(char *, HMMIO *, WAVEFORMATEX **, MMCKINFO *) { return RGF_FAILURE; }
int StreamingAudio::WaveRead(HMMIO, int, BYTE *, MMCKINFO *, UINT *) { return RGF_FAILURE; }
...
// changed RF064
	...
	if(stricmp((szFileName+len),".mp3") == 0)
	{
		...
		MMRESULT nTimer = timeSetEvent(125, 5, &TimerFunction, (DWORD_PTR)this,
