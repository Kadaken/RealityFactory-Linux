...
#ifndef _WIN32
extern "C" int geSound_SetMasterVolume(geSound_System *sound_system,
                                        float volume);

static geSound_System *rf_mixer_sound_system = NULL;
static DWORD rf_mixer_master_value = 65535u;

void RF_MixerBindSoundSystem(geSound_System *sound_system)
{
	rf_mixer_sound_system = sound_system;
}

CMixer::CMixer(DWORD DstType, DWORD SrcType, DWORD ControlType) :
	m_HMixer(NULL),
	m_iMixerControlID((INT)ControlType),
	mmr(0),
	m_dwChannels(SrcType),
	m_bSuccess(TRUE)
{
	(void)DstType;
}
...
{
	return rf_mixer_master_value;
}

BOOL CMixer::SetControlValue(DWORD value)
{
	if (value > 65535u)
		value = 65535u;
	if (m_dwChannels != NO_SOURCE)
		return TRUE;

	rf_mixer_master_value = value;
	if (rf_mixer_sound_system == NULL)
		return FALSE;
	return geSound_SetMasterVolume(rf_mixer_sound_system,
		(float)value / 65535.0f) ? TRUE : FALSE;
}

BOOL CMixer::On()
{
	return SetControlValue(65535u);
}

BOOL CMixer::Off()
{
	return SetControlValue(0u);
}
