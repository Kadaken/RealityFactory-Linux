...
CLevelController::CLevelController()
{
		...
		pEntity=geEntity_EntitySetGetNextEntity(pSet, pEntity))
	{
		...
		{
			pLC->szEntityName = (char *)geEntity_GetNameString(pEntity);
		}
		...
		if(!EffectC_IsStringNull(pLC->ScriptName))
		{
			...
			if(!EffectC_IsStringNull(pLC->InitOrder))
			{
				...
				try
				{
					skExecutableContext context = CCD->GetskContext();
					Object->method(skString(Object->Order), args, ret, context); //change simkin
				}
				catch(skRuntimeException e)
				{
					char szBug[256];
	...
	if(pSet)
	{
			...
			pEntity=geEntity_EntitySetGetNextEntity(pSet, pEntity))
		{
			...
			if(!EffectC_IsStringNull(Object->Order))
			{
				...
				{
					skExecutableContext context = CCD->GetskContext();
					Object->method(skString(Object->Order), args, ret, context);
				}
				catch(skRuntimeException e)
				{
					char szBug[256];
