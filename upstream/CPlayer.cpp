...
CPlayer::CPlayer()
{
	/* 04/30/2004 Wendell Buckner: guard missing entity/parameter teardown.
	   Native port: move the actor default ahead of all early returns. */
	// Defaults-file failure returns from this constructor before LoadAvatar.
	// Teardown must remain safe even when no actor was ever loaded.
	Actor = NULL;
...
// changed RF063
		...
		if(!pSet || !geEntity_EntitySetGetNextEntity(pSet, NULL))
		{
			CCD->ReportError("[ERROR] CPlayer::LoadAvatar: missing PlayerSetup entity", false);
			// InitializeLevel owns failure propagation; never delete this or
			// the global manager graph while this member function is running.
			return RGF_FAILURE;
		}
