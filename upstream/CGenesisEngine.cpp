...
static LRESULT HostWindowEvent(HWND, UINT message, WPARAM, LPARAM)
{
    if (!CCD) return 0;
    if (message == WM_TIMER) {
        if (CCD->NetPlayerManager()) CCD->NetPlayerManager()->ServerClientCycle();
    } else if (message == WM_SETFOCUS || message == WM_KILLFOCUS) {
        const bool focused = message == WM_SETFOCUS;
        CCD->SetHasFocus(focused);
        if (CCD->Engine()) geEngine_Activate(CCD->Engine()->Engine(), focused);
    } else if (message == WM_WINDOWPOSCHANGED && CCD->Engine()) {
							   ...
							   bool UseDialog, char *szStartLevel)
{
    ...
    m_nWidth = nWidth;
    m_nHeight = nHeight;
    // Automated boots stay windowed even if an old config requests fullscreen.
    m_fFullScreen = RF_HostHumanLaunch() && fFullScreen;
    m_MasterVolume = 0;
    m_SplashAudio = NULL;
    m_World = NULL;
    m_theEngine = NULL;
    ...
    m_CurrentLevel[0] = 0;
    m_SelectedDriverID = 'O'; // Native inherited OpenGL driver, not Direct3D.
    (void)chDriverID;
    fFogStart = fFogEnd = 0.0f;
    if (UseDialog) {
        OPENFILENAME dialog;
        memset(&dialog, 0, sizeof(dialog));
        GetOpenFileName(&dialog); // Explicit unsupported-operation diagnostic.
    }
    m_wndMain = RF_HostCreate(szName, nWidth, nHeight, HostWindowEvent);
    if (!m_wndMain) throw std::runtime_error("RF SDL window creation failed");
...
// End Dee
	...
	{
		RF_HostBindEngine(NULL);
		if (m_theEngine) geEngine_Free(m_theEngine);
		RF_HostDestroy(m_wndMain);
		throw std::runtime_error("Genesis3D engine creation failed; check driver diagnostics");
	}
	...
	if(m_Audio == NULL)
	{
		...
		RF_HostDestroy(m_wndMain);
		throw std::runtime_error("Genesis3D audio initialization failed; check OpenAL device");
	}

	geSound_SetMasterVolume(m_Audio, 0.99f);			// 100% audio for now.
	RF_MixerBindSoundSystem(m_Audio);
	ShowWindow(m_wndMain, SW_SHOWNORMAL); // Host refuses visibility without human-launch opt-in.
...
CGenesisEngine::~CGenesisEngine()
{
	...
	RF_HostDestroy(m_wndMain); // Context first, then SDL window, then SDL_Quit.
...
bool CGenesisEngine::CreateEngine(const char *szName)
{
	...
	const char *driverDirectory = RF_HostDriverDirectory();
	if (!driverDirectory) return false;
	m_theEngine = geEngine_Create(m_wndMain, szName, driverDirectory);
	RF_HostBindEngine(m_theEngine);
...
bool CGenesisEngine::LoadLevel(const char *szLevelFileName)
{
	...
	geEntity_EntitySet *schema = RF_DeclareEntitySchemas();
	if(!schema)
	{
	...
	m_World = geWorld_CreateWithEntitySchema(m_Level, schema);
	...
	if(!RF_ReserveEntitySchemas(m_World))
	{
		geWorld_Free(m_World);
		m_World = NULL;
		return FALSE;
...
{
	fprintf(stderr, "%s\n", szError); // Startup errors must remain visible before debug logging initializes.
	FILE *fd;
